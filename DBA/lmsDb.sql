...
create table employee(empId VARCHAR(255) NOT NULL , empName VARCHAR(255), empPassword VARCHAR(255), salary DOUBLE(12,2), joinDate Date, branchId INT(4), PRIMARY KEY(empId), FOREIGN KEY (branchId) REFERENCES branch(branchId) ); 
...
create table exam(examId INT(4) NOT NULL AUTO_INCREMENT, examName VARCHAR(255), examDate DATE, courseId INT(4), PRIMARY KEY(examId,examDate),  FOREIGN KEY (courseId) REFERENCES course(courseId) );
